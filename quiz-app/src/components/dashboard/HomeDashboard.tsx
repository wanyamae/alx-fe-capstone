...
export default function HomeDashboard() {
  ...
  const quizzes = useSelector((state: RootState) => state.user?.quizzes || []);
  const attempted = useSelector((state: RootState) => state.user?.quizzes?.length || 0);
  return (
    <div className={`mouse-pointer p-6 mt-10 ${dark ? 'mouse-pointer bg-gray-900 text-gray-100' : ''}`}>
